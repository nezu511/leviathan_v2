...
use crate::my_trait::leviathan_trait::{State, TransactionExecution, RoleBack, MessageCall, CompiledContract};
...
impl MessageCall for LEVIATHAN {
    fn message_call(&mut self, state: &mut WorldState, substate: &mut SubState, sender: Address, origin: Address, recipient: Address, contract: Address,
                    gas: U256, price: U256, eth: U256, apparent_value: U256, data: Vec<u8>, depth: usize, sudo: bool, block_header: &BlockHeader
        ...
        let mut execution_environment = ExecutionEnvironment::new(recipient.clone(),origin.clone(), price, data, sender.clone(), apparent_value, 
                                                            Vec::new(), block_header, depth, sudo);

        //プリコンパイル判定と実行の要件
        let contract_u256 = contract.to_u256();
        let result = match contract_u256 {
            val if val == U256::from(1) => todo!(),  //ECDSA公開鍵復元
            val if val == U256::from(2) => {        //SHA256
                LEVIATHAN::sha256(gas, &execution_environment.i_data)
            },

            val if val == U256::from(3) => {        //RIP160
                LEVIATHAN::precompile_ripemd160(gas, &execution_environment.i_data)
            },

            val if val == U256::from(4) => {        //ID: 入力データをそのまま返す
