...
mod state_tests {
    ...
    #[test]
    fn state_test() {
        // 🌟 1024階層の再帰（スタックオーバーフロー）を回避するため、64MBの巨大なスタックを持つ専用スレッドを構築
        ...
        let handler = builder.spawn(|| {
            ...
            // 🌟 テストしたいディレクトリを配列で複数指定
            let test_dirs = vec![
                "GeneralStateTestsFiller/stCallCodes",
                "GeneralStateTestsFiller/stCallCreateCallCodeTest",
                "GeneralStateTestsFiller/stCallDelegateCodesCallCodeHomestead",
                "GeneralStateTestsFiller/stCallDelegateCodesHomestead",
                "GeneralStateTestsFiller/stCreate2",
                "GeneralStateTestsFiller/stCreateTest",
                "GeneralStateTestsFiller/stInitCodeTest",
                "GeneralStateTestsFiller/stMemoryStressTest",
                "GeneralStateTestsFiller/stMemoryTest",
                "GeneralStateTestsFiller/stRefundTest",
                "GeneralStateTestsFiller/stRevertTest",
                "GeneralStateTestsFiller/stSStoreTest",
                "GeneralStateTestsFiller/stSolidityTest",
                "GeneralStateTestsFiller/stWalletTest",
                // 実行したいディレクトリをここに追加できます
            ];
                ...
                for path in paths {
                    ...
                    for (test_name, test_data) in suite.tests {
                        ...
                        for (expect_idx, expect_data) in test_data.expect.iter().enumerate() {
                            ...
                            let tx_data_str = test_data.transaction.data.get(data_idx).unwrap_or_else(|| {
                                test_data.transaction.data.first().unwrap_or(&default_str)
                            });
                            let gas_limit_str = test_data.transaction.gas_limit.get(gas_idx).unwrap_or_else(|| {
                                test_data.transaction.gas_limit.first().unwrap_or(&default_str)
                            });
                            let value_str = test_data.transaction.value.get(value_idx).unwrap_or_else(|| {
                            ...
                            for (addr_str, expected_acc) in &expect_data.result {
                                ...
                                if let Some(expected_nonce_str) = &expected_acc.nonce {
                                    let expected_nonce: u32 = // 🌟 u64に変更済み！
        ...
        // スレッドの終了を待機し、パニックが発生した場合はテストを失敗させる
        handler.join().unwrap();
    }
}
